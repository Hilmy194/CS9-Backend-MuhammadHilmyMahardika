...
exports.deleteStore = async (id) => {
  try {
    // Ambil data store terlebih dahulu untuk dikembalikan sebagai payload
    const store = await exports.getStoreById(id);
    if (!store) return { rowCount: 0 };
    const res = await db.query("DELETE FROM stores WHERE id = $1", [id]);
    return { rowCount: res.rowCount, payload: store };
  } catch (error) {
    console.error("Error executing query", error);
  }
};
